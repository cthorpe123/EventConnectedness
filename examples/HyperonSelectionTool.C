...
#include "TTree.h"
   ...
   void HyperonSelectionTool(std::string infile,std::string intracklist){
      ...
      std::ifstream tracklist(intracklist);
      ...
      for(size_t j=0;j<nentries;j++){
         ...
         if(ClusterIDs_Plane0.size() == 3 && ClusterIDs_Plane0.at(0) == 1 && ClusterIDs_Plane0.at(1) == 2 && ClusterIDs_Plane0.at(2) == 2 && ClusterSizes_Plane0.at(0) > min_size && ClusterSizes_Plane0.at(1) > min_size && ClusterSizes_Plane0.at(2) > min_size) Planes_Passed++; 
         ...
         // Selection criteria: ID vector should read 1 2 2 (muon and proton/pion merged) 
         // Clusters should all be of at least size min_size
         if(ClusterIDs_Plane1.size() == 3 && ClusterIDs_Plane1.at(0) == 1 && ClusterIDs_Plane1.at(1) == 2 && ClusterIDs_Plane1.at(2) == 2 && ClusterSizes_Plane1.at(0) > min_size && ClusterSizes_Plane1.at(1) > min_size && ClusterSizes_Plane1.at(2) > min_size) Planes_Passed++; 
         ...
         // Selection criteria: ID vector should read 1 2 2 (muon and proton/pion merged) 
         // Clusters should all be of at least size min_size
         if(ClusterIDs_Plane2.size() == 3 && ClusterIDs_Plane2.at(0) == 1 && ClusterIDs_Plane2.at(1) == 2 && ClusterIDs_Plane2.at(2) == 2 && ClusterSizes_Plane2.at(0) > min_size && ClusterSizes_Plane2.at(1) > min_size && ClusterSizes_Plane2.at(2) > min_size) Planes_Passed++; 

         c.Reset();

         if(Planes_Passed > 0){ /*std::cout << "Selected Event" << std::endl;*/ selected_events++; }
         total_events++;
